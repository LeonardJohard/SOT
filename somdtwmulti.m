classdef somdtw
    ...
    %adaptDTW(obj, steps): Updates the assignment of trajectory points to nodes
    %adapt(obj, steps): Updates node positions to assigned trajecory points
    %ewma(obj, pos, q): Update the moving average and variance estimates
    %(run adaptDTW to trigger this function)
   ...
   methods 
      function  obj = somdtw(examplepos, nrnodes)
          ...
          obj.segvar = 0.001*ones(obj.indim,nrnodes-1);
      end
      ...
      function obj = ewma(obj, pos, q) %exponentially weighted moving average
          ...
          for i = 1:length(q)
           ...
           if 1 >= r && r >= 0 %Point within line segment
             nodebins(:,currentseg(winnernode)) = nodebins(:,currentseg(winnernode)) + ones(obj.indim,1);
             nodesums(:,currentseg(winnernode)) = nodesums(:,currentseg(winnernode)) + distance;
           end
      ...
      function [obj totalnodeavg totalnodevar] = fullmean(obj)
          ...
          for i = 1:obj.nrseries
              ...
              nodemean{i}(:,:) = nan(indim,obj.nrnodes-1);
